...
<div class="col-sm-12 col-xl-10">
    ...
    <div id="outil-fenetre">
        ...
        <? if ( ! empty($variables) && $type == 9) : ?>

            <div id="variables-titre">
            
                Variable<?= count($variables) > 1 ? 's' : ''; ?> aléatoire<?= count($variables) > 1 ? 's' : ''; ?> :
            ...
            <div id="variables">
                ...
                <?
                    foreach($variables as $var => $val) : 
                        ...
                        if ( ! preg_match('/<var>' . $var . '<\/var>/', $question_originale) && ! preg_match('/' . $var . '/', $reponse['reponse_texte'])) 
                            continue;
                ?>

                    <div class="btn btn-outline-secondary" style="color: #444; border-color: #ccc">
                        <?= $var; ?> = <?= str_replace('.', ',', $val); ?>
                    </div>

                    <input name="var[]" type="hidden" value="<?= $var . '_' . $val; ?>">

                <?
                    endforeach;
                ?>

                <div class="btn btn-outline-secondary spinnable" style="margin-left: 10px;" onclick="document.location.reload(true);">
                    <i class="fa fa-random" style="margin-right: 5px"></i>
        ...
        <? if ($reponse['equation']) : ?>
            ...
            <div id="equation">
                ...
                <? if (empty($reponse['cs']) || $reponse['cs'] == 0) : ?>
                    <span style="margin-left: 10px; color: #1565C0">(auto CS)</span>
                <? elseif ($reponse['cs'] != 99) : ?>
                    <span style="margin-left: 10px">(<?= $reponse['cs']; ?> CS)</span>
                <? else : ?>
                    <span style="margin-left: 10px">(<i class="fa fa-times"></i> CS)</span>
                <? endif; ?>
                
                <input name="reponse_id" type="hidden" value="<?= $reponse['reponse_id']; ?>">
                ...
                <tr>
                    <td>
                        <div class="input-group" style="width: 325px">
                            ...
                            <input name="bonne_reponse" type="hidden" value="<?= $reponse_correcte; ?>">

                            <div id="bonne-reponse" type="text" class="form-control" value="<?= $reponse_correcte; ?>" style="text-align: right; background: #f7f7f7">
                                <?= $reponse_correcte; ?>
                            </div>
                    ...
                    <td>
                        <span style="margin-left: 15px">
                            Réponse brute = <?= $reponse['reponse_brute']; ?> <?= $reponse['unites'] ?: ''; ?>
                        </span>
                    </td>
                </tr>
                ...
                <tr class="tolerance"> 
                    ...
                    <td>
                        <div style="margin-left: 15px">
                            <? if ($t['type'] == 2) : ?>
                                Absolue = 
                                <?= str_replace('.', ',', (str_replace(',', '.', $reponse_correcte) * str_replace(',', '.', $t['tolerance']) / 100)); ?>
                                <?= $reponse['unites'] ?: 'u'; ?>
                            <? endif; ?>
                        </div>
                    </td>
