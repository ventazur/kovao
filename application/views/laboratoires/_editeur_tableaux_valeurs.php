...
<div id="editeur-tableaux" class="mt-4 mb-4">
    ...
    <div id="editeur-tableaux-titre" class="editeur-section-titre">
        ...
        Tableaux : valeurs de comparaison
    </div>
        ...
        <? else : ?>
            ...
            <table class="table table-sm table-borderless" style="margin: 0; font-size: 0.9em">
                ...
                <tbody>
                    ...
                    <?  foreach(range(1, $tableau_max) as $no_tableau) : ?>

                        <? if ( ! array_key_exists($no_tableau, $v_par_t)) continue; ?>
                        ...
                        <?  
                            ...
                            if ( ! empty($lab_points_tableaux) && array_key_exists($no_tableau, $lab_points_tableaux) && is_array($lab_points_tableaux[$no_tableau]))
                            {
                                foreach($lab_points_tableaux[$no_tableau] as $c)
                                {
                        ...
                        <tr>
                            <? if ($no_tableau == 1) : ?>
                                <td colspan="10">
                                    <div class="mb-2" style="padding: 10px; background: #B2DFDB; color: #006064; border-radius: 3px;">
                                        Tableau 1
                                    </div>
                                </td>
                            <? endif; ?>
                            ...
                            <? if ($no_tableau > 1) : ?>
                                <td colspan="10">
                                    <div class="mt-2 mb-2" style="padding: 10px; background: #B2DFDB; color: #006064; border-radius: 3px;">
                                        Tableau <?= $no_tableau; ?>
                                    </div>
                                </td>
                            <? endif; ?>
                        ...
                        <? foreach($v_par_t[$no_tableau] as $c => $c_arr) : ?>
                            ...
                            <tr>
                                <td class="text-left">
                                    <? if ($evaluation['enseignant_id'] == $this->enseignant_id || permis('admin_lab')) : ?>
                                        <div class="modal-tableau-modifier-champ btn btn-sm btn-light" data-toggle="modal" data-target="#modal-tableau-modifier-champ" style="color: #007bff"
                                             data-evaluation_id="<?= $evaluation_id; ?>" data-champ="<?= $c; ?>" data-champ_d="<?= ajouter_d_champ($c); ?>">
                                ...
                                <td class="text-left">
                                    <? if (array_key_exists('est_incertitude', $lv[$c]) && ! empty($lv[$c]['valeur'])) : ?>
                                        <span class="tag-champ" style="font-size: 0.9em; <?= $lv[$c]['est_incertitude'] ? 'background: #9575CD; color: #fff' : ''; ?>"><?= $c; ?></span>
                                    <? endif; ?>
                                </td>
                                ...
                                <td>
                                    <? if (array_key_exists($c, $lab_points)) : ?>
                                        <span style="font-family: Lato; font-weight: 300;"><?= $lab_points[$c]['desc']; ?></span>
                                    <? endif; ?>
                                </td>
                                ...
                                <td class="text-right">
                                    <? if ( ! empty($lv[$c]['valeur'])) : ?>
                                        <div class="btn btn-light">

                                            <? if (array_key_exists('est_incertitude', $lv[$c]) && $lv[$c]['est_incertitude']) : ?>
                                                <span>±</span>
                                            <? endif; ?>

                                            <span
                                                id="<?= $lab_prefix . $c; ?>"
                                                data-name="<?= $lab_prefix . $c; ?>"
                                                data-champ="<?= $c; ?>"
                                                data-value="<?= str_replace('.', ',', $lv[$c]['valeur']); ?>"
                                            >
                                                <?= str_replace('.', ',', $lv[$c]['valeur']); ?>
                                            </span>

                                        </div>
                                    <? endif; ?>

                                    <? if ( ! empty($lv[$c]['unites'])) : ?>
                                        <div class="btn btn-light" disabled>
                                            <?= ns_seul_format($lv[$c]['nsci']); ?>
                                            <?= $lv[$c]['unites']; ?>
                                        </div>
                                    <? endif; ?>
                                </td>
